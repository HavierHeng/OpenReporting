...
func main() {
	...
	http.HandleFunc("/api/fields", func(w http.ResponseWriter, r *http.Request) {
		...
		// }
		fmt.Fprintf(w, `
			<div class="chat-bubble slide-in">
				<p class="text-lg font-semibold mb-2">------ Static Fields ------</p>
				<div class="form-container">
					<div class="form-group">
						<label for="client_name" class="form-label">Client Name</label>
						<input type="text" id="client_name" name="client_name" value="" class="form-input" placeholder="Enter client name">
						<span class="text-xs text-muted">Full name of the client</span>
					</div>
					<div class="form-group">
						<label for="report_date" class="form-label">Report Date</label>
						<input type="date" id="report_date" name="report_date" value="2025-05-19" class="form-input">
						<span class="text-xs text-muted">Date of the report (YYYY-MM-DD)</span>
					</div>
						...
						<div class="mt-2 space-y-2">
							<template x-for="row in rows" :key="row.id">
								<div class="flex space-x-2">
									<input type="text" x-model="row.item" class="form-input flex-1" placeholder="Item">
									<input type="number" x-model="row.quantity" class="form-input w-24" placeholder="Quantity">
								</div>
							</template>
						</div>
						<button @click="rows.push({ id: rows.length + 1, item: '', quantity: '' })" class="mt-2 form-button">+ Add Row</button>
					</div>
					<button hx-post="/api/save-static" hx-include="[name='client_name'],[name='report_date'],[x-model]" hx-target="#chat-history" hx-swap="beforeend" @click="startConversation" class="mt-4 form-button">Submit Static Fields</button>
				</div>
			</div>
		`)
	})
	...
	http.HandleFunc("/api/save-static", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `
			<div class="chat-bubble slide-in">
				...
				<p>I will now assist you with filling in finances through a series of questions.</p>
			</div>
		`)
	})
	...
	http.HandleFunc("/api/chat", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `
			<div class="chat-bubble user slide-in">
				<p>%s</p>
			</div>
			<div class="chat-bubble slide-in">
				<p>Got it. Proposed value: "%s" for finances. Confirm?</p>
				<button hx-post="/api/confirm-field" hx-vals='{"field": "finances", "value": "%s"}' hx-target="#chat-history" hx-swap="beforeend" class="mt-2 form-button">Confirm</button>
			</div>
		`, input, input, input)
	})
	...
	http.HandleFunc("/api/confirm-field", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `
			<div class="chat-bubble slide-in">
				...
				<p>Summary: The report covers financial performance for Q2 2025 in Japan. Confirm and download?</p>
				<button hx-post="/api/generate-report" hx-target="#chat-history" hx-swap="beforeend" class="mt-2 form-button">Confirm & Download</button>
			</div>
		`)
	})
